<script lang="ts">
    ...
    // بيانات الرسم الدائري لتوزيع السيولة
    $: liquidityData = data ? data.liquidity
        .filter((l) => Number(l.balance) > 0)
        .slice(0, 6)
        .map((l, i) => {
            const colors = ['#3b82f6', '#00d68f', '#a855f7', '#f5a623', '#06b6d4', '#ff5252'];
            return { label: l.asset, value: Number(l.balance), color: colors[i % colors.length] };
        }) : [];

    $: liquidityTotal = liquidityData.reduce((s, d) => s + d.value, 0);

    // بيانات الرسم الشريطي للعمليات
    $: statsData = data ? [
        { label: 'مستخدمون', value: data.users, color: '#3b82f6' },
        { label: 'أوامر', value: data.orders.total, color: '#00d68f' },
        { label: 'مفتوحة', value: data.orders.open, color: '#f5a623' },
        { label: 'صفقات', value: data.trades, color: '#a855f7' },
        { label: 'إيداعات', value: data.pending.deposits, color: '#06b6d4' },
        { label: 'سحوبات', value: data.pending.withdrawals, color: '#ff5252' },
    ] : [];

    $: spotTrades = data ? data.trades : 0;
</script>
...
<div class="space-y-6">
    <div>
        ...
        <p class="text-sm text-text-secondary mt-1">الحالة اللحظية والمؤشرات الأساسية للنظام</p>
    </div>

    {#if loading}
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {#each Array(6) as _}
                <div class="card-compact animate-pulse">
                    <div class="h-3 bg-base-700 rounded w-2/3 mb-2"></div>
                    <div class="h-6 bg-base-700 rounded w-1/2"></div>
                </div>
            {/each}
        </div>
    {:else if error}
    ...
    {:else if data}
        ...
        <section class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            <div class="card-compact hover:border-accent-blue transition-colors">
            ...
            <div class="card-compact hover:border-accent-green transition-colors">
            ...
            <div class="card-compact hover:border-accent-yellow transition-colors">
            ...
            <div class="card-compact {data.circuit_breaker_open ? 'border-accent-red' : ''}">
        ...
        <section class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div class="card-default">
                <h2 class="text-sm font-semibold text-text-primary uppercase tracking-wider mb-4">توزيع السيولة</h2>
                <div class="flex items-center gap-6 flex-wrap">
                    <DonutChart
                        ...
                        centerValue={fmtNum(liquidityTotal, 0)}
                    />
                    <div class="flex-1 min-w-[180px] space-y-2">
                        {#each liquidityData as d}
                            <div class="flex items-center justify-between text-sm">
                                <div class="flex items-center gap-2">
                                    <span class="w-3 h-3 rounded-full" style="background: {d.color}"></span>
                                    <span class="text-text-secondary">{d.label}</span>
                                </div>
                                <span class="num-cell text-text-primary font-medium">
                                    {d.label === 'EGP' ? fmtEgp(d.value) : fmtQty(d.value.toString(), 4)}
                                </span>
                            </div>
                        {/each}
                    </div>
            ...
            <!-- رسم إحصائي شريطي -->
            <div class="card-default">
                <h2 class="text-sm font-semibold text-text-primary uppercase tracking-wider mb-4">إحصائيات النظام</h2>
                <BarChart data={statsData} height={220} />
            </div>
        </section>

        <!-- جدول السيولة التفصيلي -->
        <section>
            <h2 class="text-sm font-semibold text-text-secondary uppercase tracking-wider mb-3">سيولة النظام (تفصيلي)</h2>
                    ...
                    <tbody>
                        {#each data.liquidity as l}
                            <tr>
                                ...
                                <td class="num-cell text-accent-yellow">{l.asset === 'EGP' ? fmtEgp(l.locked) : fmtQty(l.locked, 8)}</td>
                                ...
                                <td class="num-cell text-text-tertiary">
                                    {#if Number(l.balance) > 0}
                                        {((Number(l.locked) / Number(l.balance)) * 100).toFixed(1)}%
                                    {:else}
                                        —
                                    {/if}
                                </td>
        ...
        <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <a href="/deposits" class="card-default hover:border-accent-yellow transition-colors block group">
                <div class="flex items-center justify-between">
                    ...
                    <div class="text-2xl font-bold text-accent-yellow group-hover:scale-110 transition-transform">{data.pending.deposits}</div>
                </div>
            </a>
            <a href="/withdrawals" class="card-default hover:border-accent-red transition-colors block group">
                <div class="flex items-center justify-between">
                    <div>
                    ...
                    <div class="text-2xl font-bold text-accent-red group-hover:scale-110 transition-transform">{data.pending.withdrawals}</div>
                </div>
            </a>
            <a href="/currencies" class="card-default hover:border-accent-blue transition-colors block group">
                <div class="flex items-center justify-between">
                    <div>
                    ...
                    <svg class="w-6 h-6 text-accent-blue group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8" /></svg>
                </div>
            </a>
            <a href="/settings" class="card-default hover:border-accent-purple transition-colors block group">
                <div class="flex items-center justify-between">
                    <div>
