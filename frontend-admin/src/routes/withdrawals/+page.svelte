<script lang="ts">
    ...
    async function submitReview() {
        ...
        if (reviewStatus === 'completed' && !txHash) {
            reviewError = 'هاش المعاملة مطلوب لإتمام السحب';
            return;
        }
...
<svelte:head><title>طابور السحوبات · الإدارة</title></svelte:head>

<div class="space-y-6">
    <div class="flex items-center justify-between flex-wrap gap-2">
        <div>
            <h1 class="text-2xl font-bold text-text-primary">طابور سحب العملات الرقمية</h1>
            <p class="text-sm text-text-secondary mt-1">معالجة طلبات السحب - بث المعاملة على البلوكتشين ثم اعتمادها</p>
        </div>
    ...
    {:else if items.length === 0}
        <div class="card-default text-center py-12 text-text-tertiary text-sm">لا توجد طلبات سحب في هذا العرض</div>
    {:else}
                ...
                <tbody>
                    {#each items as item}
                        <tr class="cursor-pointer" on:click={() => openReview(item.tx)}>
                            ...
                            <td class="font-bold">{item.tx.asset_symbol}</td>
    ...
    <div class="fixed inset-0 z-50 bg-base-900/70 flex items-center justify-center p-4" on:click|self={() => (selected = null)}>
        <div class="bg-base-800 border border-base-600 rounded-lg w-full max-w-lg p-6">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-semibold text-text-primary">مراجعة السحب</h3>
            ...
            <div class="space-y-3 text-sm">
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div class="col-span-2">
                        <div class="text-xs text-text-tertiary uppercase">عنوان الوجهة</div>
                ...
                <div>
                    ...
                    <select id="status" bind:value={reviewStatus} class="input">
                        ...
                        <option value="approved">موافق (قيد التنفيذ)</option>
                        <option value="completed">مكتمل (بُثت على البلوكتشين)</option>
                        <option value="rejected">مرفوض (استرجاع المبلغ المحجوز)</option>
                ...
                {#if reviewStatus === 'completed'}
                    <div>
                        <label class="label" for="hash">هاش المعاملة على البلوكتشين <span class="text-accent-red">*</span></label>
                        <input id="hash" type="text" bind:value={txHash} class="input" placeholder="0x..." dir="ltr" />
                    </div>
                {/if}
