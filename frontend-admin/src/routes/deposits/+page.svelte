<script lang="ts">
    ...
    async function load() {
        try {
            const params = filter === 'all' ? { tx_type: 'deposit' as const } : { tx_type: 'deposit' as const, status: filter };
...
<svelte:head><title>Deposits Queue · Admin</title></svelte:head>

<div class="space-y-6">
    <div class="flex items-center justify-between flex-wrap gap-2">
        <div>
            <h1 class="text-2xl font-bold text-text-primary">EGP Deposits Queue</h1>
            <p class="text-sm text-text-secondary mt-1">All manual deposit requests, oldest first</p>
        </div>
    ...
    {:else if filteredItems.length === 0}
        <div class="card-default text-center py-12 text-text-tertiary text-sm">No deposit requests in this view</div>
    {:else}
    ...
    <div class="fixed inset-0 z-50 bg-base-900/70 flex items-center justify-center p-4" on:click|self={() => (selected = null)}>
        <div class="bg-base-800 border border-base-600 rounded-lg w-full max-w-lg p-6">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-lg font-semibold text-text-primary">Review Deposit</h3>
            ...
            <div class="space-y-3 text-sm">
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <div class="num-cell text-text-secondary">{selected.user_id}</div>
                    </div>
                    <div>
                    ...
                    <div>
                        <div class="text-xs text-text-tertiary uppercase">Reference</div>
                ...
                <div>
                    ...
                    <select id="status" bind:value={reviewStatus} class="input">
                        ...
                        <option value="approved">Approved (credit pending)</option>
                        <option value="completed">Completed (credit wallet now)</option>
                ...
                <div>
                    ...
                    <textarea id="note" bind:value={reviewNote} class="input min-h-[80px]" placeholder="Optional note visible to user..."></textarea>
                </div>
